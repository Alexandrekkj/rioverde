...
function AnalisadorIA({
  ...
  const gerarAnalise = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("analise-financeira", {
        body: { periodo, dados },
      });
      ...
      toast.error("Erro ao gerar análise: " + (e?.message ?? "desconhecido"));
    } finally {
      setLoading(false);
    }
  }, [periodo, dados]);
      ...
      <CardContent className="space-y-3">
        {analise ? (
          ...
          <p className="text-sm text-muted-foreground">
            Clique em "Gerar Análise" para obter um resumo interpretativo inteligente do período.
          </p>
        )}
        <Button
          size="sm"
...
function PainelDiario() {
  ...
  const evolucao7d = useMemo(() => {
    ...
    return days.map((d) => {
      ...
      const vDia = vendas7.filter((v: any) => format(new Date(v.data), "yyyy-MM-dd") === key);
      const iDia = itens7.filter((i: any) => vDia.some((v: any) => v.id === i.venda_id));
      const dDia = despesas7.filter((d2: any) => format(new Date(d2.data), "yyyy-MM-dd") === key);
      const f = calcFinanceiro(vDia, iDia, dDia);
      return { date: format(d, "dd/MM", { locale: ptBR }), lucro: f.lucroLiquido, receita: f.receita };
    });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Resumo do Dia</CardTitle></CardHeader>
          <CardContent>
            <ChartContainer config={{ valor: { label: "R$", color: "hsl(var(--primary))" } }} className="h-[200px]">
...
function PainelSemanal() {
  ...
  const evolucaoDiaria = useMemo(() => {
    ...
    return days.map((d) => {
      const key = format(d, "yyyy-MM-dd");
      const vDia = vendas.filter((v: any) => format(new Date(v.data), "yyyy-MM-dd") === key);
                ...
                {rankingProdutos.map((p, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant={i < 3 ? "default" : "secondary"} className="w-5 h-5 justify-center text-xs p-0">{i + 1}</Badge>
                      <span className="text-sm truncate max-w-[130px]">{p.nome}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-primary">{fmt(p.lucro)}</p>
...
function PainelMensal() {
  ...
  const crescimentoReceita = finAnt.receita > 0 ? ((fin.receita - finAnt.receita) / finAnt.receita) * 100 : 0;
  const crescimentoLucro = finAnt.lucroLiquido > 0 ? ((fin.lucroLiquido - finAnt.lucroLiquido) / finAnt.lucroLiquido) * 100 : 0;

  // Projeção: dias passados vs dias do mês
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Badge variant="outline" className="text-xs gap-1">
          Status: <span className="font-bold">{statusNegocio}</span>
        </Badge>
        <Badge variant="outline" className="text-xs">
          {diaAtual}/{totalDias} dias do mês
        </Badge>
      </div>
      ...
      <Card className="border-primary/30 bg-primary/5">
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground mb-2 font-medium">📊 Projeção ao final do mês (mantendo ritmo atual)</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-muted-foreground">Receita projetada</p>
              <p className="text-lg font-bold text-primary">{fmt(projecaoReceita)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Lucro líq. projetado</p>
              <p className={`text-lg font-bold ${projecaoLucro >= 0 ? "text-primary" : "text-destructive"}`}>{fmt(projecaoLucro)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="space-y-2">
                {topClientes.map((c, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant={i < 3 ? "default" : "secondary"} className="w-5 h-5 justify-center text-xs p-0">{i + 1}</Badge>
                      <span className="text-sm truncate max-w-[130px]">{c.nome}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-primary">{fmt(c.lucro)}</p>
...
export default function DashboardEstrategico() {
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold">Dashboard Estratégico</h1>
        <p className="text-xs text-muted-foreground mt-0.5">Análise financeira com lucro real e resumos inteligentes</p>
      </div>
